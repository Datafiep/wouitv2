...
def futures_page():
    ...
    show_chart = st.sidebar.checkbox("Show Price Trend Chart")
    ...
    def display_data():
        ...
        # Display the interactive chart if the checkbox is selected
        if show_chart:
            fig = px.line(df_wheat_prices, x='Time', y='Last', title='Wheat Price Trend')
            st.plotly_chart(fig)
